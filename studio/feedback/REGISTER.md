...
| 2026-07-01-city-music-one-tone-not-melody | bug | assessed | Per-town "music" is a sustained DRONE (transposition-first), not a melody — reads as one tone. Root-caused in `src/town-theme.js`; fix = per-town melodies → attached to #129 (from-owner) | [#129](https://github.com/cakuki/tidewake/issues/129) |
| 2026-07-01-battle-not-fun-readability-isolation-diegetic | feedback | accepted | Marquee battle (#135) not fun: occluding popup, unclear target, rescue interferes, no visible cannonballs/angles, wants diegetic hover-interaction. GD+TL diagnosed presentation gap → **epic #161** (6 slices), recommend preempting-lane | [#161](https://github.com/cakuki/tidewake/issues/161) |
| 2026-07-01-pause-game | feature | accepted | Pause the game (freeze sim + PAUSED overlay + resume); S, BAU-friendly | [#160](https://github.com/cakuki/tidewake/issues/160) |
| 2026-07-02-qa-must-play-quality-over-features | feedback | triaging | **SYSTEMIC:** QA must PLAY/investigate (not just gates) — recurring quality misses (playdough harbours, pencil NPC ships, one-tone music, ships-through-islands, beaching, un-angled combat, rescue interrupts). + "improve existing before new". Wins to reinforce: name-tags/props/fame. Exploratory-QA + doctrine passes running → bug issues | — (bugs → issues) |
| 2026-07-02-remove-olive-buoy-marker | bug | accepted | Owner: "martini olive flag carrier" reads as junk next to the hero ship → remove it. Root-caused = the port buoy marker (sphere+pole+pennant) in `src/ports.js` | [#183](https://github.com/cakuki/tidewake/issues/183) |
| 2026-07-02-ship-catalog-progression | feature | accepted | Ship catalog + "start small, crave the best" progression (buy bigger/better ships) + source CC0 ship models → attached to #141 (shipyards); rides #32/#143 art track | [#141](https://github.com/cakuki/tidewake/issues/141) |
| 2026-07-01-fun-and-working-over-fast-delivery | feedback | accepted | **STANDING DOCTRINE:** Fun & Working over fast delivery — "working" ≠ done; ship the visible/felt fun + progression payoff. Research fun from gamer reviews. Embedding in CONSTITUTION/LOOP DoD/rituals R2/ROADMAP + `docs/design/what-makes-it-fun.md` | — (doctrine + research) |
| 2026-07-01-headless-loop-runner-and-usage-scheduler | feature | accepted | Headless `claude` sprint runner (→ human+machine changelog linked to commits) + deterministic usage-aware scheduler (fill 5h window ~80%→90%) + separate `LOOP-SPRINT.md` A/B runbook + curated lean-plugin profile. **IMPLEMENTED** `9614e3a` (dry-run default, running-loop guard, never auto-fires); `LOOP.md` untouched. #152 open pending owner's first live run + BUDGET calibration | [#152](https://github.com/cakuki/tidewake/issues/152) |
