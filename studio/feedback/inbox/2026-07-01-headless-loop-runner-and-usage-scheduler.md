...
- 2026-07-01 — Owner: investigate the `pause-loop-await-direction` session + use a SEPARATE runbook (A/B). Found the plugin lever = `enabledPlugins` in `~/.claude/settings.json` (all 15 on). 2nd TL pass: **do NOT use `--bare`** (forces API-key billing → kills the 5h window); trim à la carte (`--setting-sources project,local` + curated `sprint.settings.json` + `--strict-mcp-config`/`--tools`/`--disable-slash-commands`); separate `LOOP-SPRINT.md` piped instead of `LOOP.md` (untouched).
- 2026-07-01 — Owner: **implement in-session** (unattended, don't block; don't start the loop from the new runner). **DELIVERED** via build subagent → commit `9614e3a`, pushed. 9 files; validated (bash -n/node --check/shellcheck/dry-run 0 claude procs/changelog round-trip). **Dry-run default + running-loop guard; never auto-fires.** #152 open pending owner's first live run + BUDGET calibration.
